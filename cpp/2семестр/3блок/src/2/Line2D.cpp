...
double Line2D::getX1P2() const {
    return m_p2.getX1();
}

double Line2D::getX2P2() const {
    return m_p2.getX2();
}

void Line2D::setX1P2(double val) 
{
    m_p2.setX2(val);
}

void Line2D::setX2P2(double val) 
{
    m_p2.setX2(val);
}
...
{
    std::cout << "Прямая, проходящая через точки " << toStr() << m_p2.toStr();
}